...
#endif
	//Added
		gpuOrb = cv::cuda::ORB::create(10000, 1.2f, 8, 31, 0, 2, 0, 31, 20, true);   
	//End added
...
void CMT::processFrame(Mat im_gray, int frame) {
    ...
    if(frame < 2 ) {
		...
		//Added
			matcher.matchGlobalAsync(keys1, desc1, points_matched_global, classes_matched_global, stream1, true);
		//End added
		double GpuTime = clock() - Gpu;
		std::cout << "Gpu time : " << GpuTime/(double)CLOCKS_PER_SEC << std::endl;
			}
    else {	
		//Detect keypoints, compute descriptors
		clock_t Gpu = clock();
    ...
    double CpuTime = clock() - Cpu;
    std::cout << "Cpu time : " << CpuTime/(double)CLOCKS_PER_SEC << std::endl;
