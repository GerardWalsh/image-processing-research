...
void Matcher::matchGlobalAsync(cv::cuda::GpuMat & keypoints, cv::cuda::GpuMat & descriptors, vector<Point2f> & points_matched, vector<int> & classes_matched, cv::cuda::Stream & stream1, bool flag)
{
    ...
    gpuOrbConvert->convert(keypoints, keypointsCpu);
    bfmatcherAsync->knnMatchConvert(matchesGpu, matches);
    stream1.waitForCompletion();
    std::cout << "Mat matches size : " << matches.size() << std::endl;
